...
class DroppableTableWidget(QTableWidget):
    ...
    def on_item_changed(self, item):
        ...
        if col == 0:
            ...
            try:
                mw = getattr(self, 'io_screen', None).main_window if getattr(
                    self, 'io_screen', None) else None
    ...
    def handle_row_click(self, row):
        ...
        if modifiers & Qt.ShiftModifier and self.last_selected_row is not None:
            ...
            cells_in_row = [self.item(row, col) for col in range(
                self.columnCount()) if self.item(row, col)]
            if not cells_in_row:
                return
    ...
    def show_force_menu(self, row, signal_name, data_type, position):
        ...
        if row in self.forced_rows:
            remove_action = QAction(
                f"[U] Remove Force from '{signal_name}'", self)
            remove_action.triggered.connect(lambda: self.remove_force(row))
            menu.addAction(remove_action)
        else:
            if data_type == 'bool':
                force_true = QAction(f"[F] Force '{signal_name}' = TRUE", self)
                force_true.triggered.connect(
                    lambda: self.apply_force(row, True))
                menu.addAction(force_true)

                force_false = QAction(
                    f"[F] Force '{signal_name}' = FALSE", self)
                force_false.triggered.connect(
                    lambda: self.apply_force(row, False))
                menu.addAction(force_false)
            else:
                force_value = QAction(
                    f"[F] Force '{signal_name}' to value...", self)
                force_value.triggered.connect(
                    lambda: self.apply_force_analog(row, signal_name))
    ...
    def manual_sort(self, column, order):
        ...
        def parse_address(addr):
            ...
            if match:
                ...
                # -1 indicates word, not bit
                return (prefix, int(word_num), -1)
        ...
        for display_row, row_content in enumerate(rows_data):
            for col in range(self.columnCount()):
                ...
                if col in [0, 2, 3]:
                    self.setItem(display_row, col,
                                 EditableTableWidgetItem(text))
                else:
                    self.setItem(display_row, col,
                                 ReadOnlyTableWidgetItem(text))
    ...
    def _clear_row_data(self, row):
        ...
        if row in self.row_data:
            if row in self.forced_rows:
                # Also clear force when clearing data
            ...
            if addr_item and addr_item.text():
                ...
                if '.' in address:
                    ...
                    if data_type and data_type.text() in ['int', 'word']:
                        try:
                            byte_num = int(address[2:]) if len(
                                address) > 2 and address[1] == 'W' else 0
                        except ValueError:
                            continue
                        prefix = address[:2]
    ...
    def find_free_address(self, prefix, data_type):
        ...
        if data_type == 'bool':
            ...
            for byte_num in range(start_byte, 254, 2):
                ...
                is_byte1_free = byte1_key not in used or len(
                    used[byte1_key]) == 0
                is_byte2_free = byte2_key not in used or len(
                    used[byte2_key]) == 0
    ...
    def dropEvent(self, event):
        ...
        if event.mimeData().hasText():
            ...
            if row >= 0 and col == 0:
                try:
                    removed_count = self.remove_duplicate_signals(
                        dropped_text, exclude_row=row)
                    signal_data = None
                    ...
                    if signal_data:
                        ...
                        if 'type' in signal_data:
                            self.setItem(
                                row, 1, ReadOnlyTableWidgetItem(data_type))
                        io_prefix = signal_data.get('io_prefix', 'I')
                        byte_num, bit_num, full_address = self.find_free_address(
                            io_prefix, data_type)
                        self.setItem(
                        ...
                        self.setItem(
                            row, 4, ReadOnlyTableWidgetItem(full_address))
                        ...
                        if 'description' in signal_data:
                            self.setItem(row, 6, ReadOnlyTableWidgetItem(
                                signal_data['description']))
                        if 'range' in signal_data:
                            self.setItem(row, 7, ReadOnlyTableWidgetItem(
                                signal_data['range']))
                        self._save_row_data(row)
                    else:
                        ...
                        self.update_status_column(
                            row, False if data_type == 'bool' else 0)
                        status_item = self.item(row, 5)
                        if status_item:
                            status_item.setBackground(
                                QColor(200, 230, 245))  # Blue
...
class IOScreen:
        ...
        if data_type == 'bool':
            ...
            for byte_num in range(start_byte, max_byte, step):
                for bit_num in range(8):
                    ...
                    in_use, _ = self.is_address_in_use(
                        address, exclude_row=exclude_row)
                    if not in_use:
    ...
    def validate_and_fix_manual_address(self, row):
        ...
        try:
            ...
            if data_type == 'bool':
                ...
                bit_num = int(
                    bit_item.text()) if bit_item and bit_item.text() else 0
            ...
            in_use, conflict_row = self.is_address_in_use(
                proposed_address, exclude_row=row)
            ...
            if in_use and conflict_row is not None:
                ...
                conflict_signal = table.item(conflict_row, 0).text(
                ) if table.item(conflict_row, 0) else "unknown"

                # Find first free address for the conflicting tag
                free_addr, free_byte, free_bit = self.find_first_free_address(
                    io_prefix, conflict_type, exclude_row=conflict_row)
                ...
                if free_addr:
                    ...
                    if not isinstance(conflict_byte_item, EditableTableWidgetItem):
                        conflict_byte_item = EditableTableWidgetItem(
                            str(free_byte))
                    ...
                    if conflict_type == 'bool' and free_bit >= 0:
                        ...
                        if not isinstance(conflict_bit_item, EditableTableWidgetItem):
                            conflict_bit_item = EditableTableWidgetItem(
                                str(free_bit))
                    ...
                    print(
                        f"[AUTO-RESOLVE] Moved '{conflict_signal}' from {proposed_address} to {free_addr}")
                else:
    ...
    def readdress_all_signals(self):
        ...
        for signal in signals_to_readdress:
            ...
            byte_num, bit_num, full_address = table.find_free_address(
                io_prefix, data_type)
...
class IOConfigMixin:
    ...
    def _replace_table_widget(self):
        ...
        try:
            ...
            self.tableWidget_IO = DroppableTableWidget(
                50, 8, parent, self.io_screen)
            headers = ["NAME", "TYPE", "BYTE", "BIT",
                       "ADDRESS", "STATUS", "DESCRIPTION", "RANGE"]
    ...
    def _connect_io_buttons(self):
        ...
        try:
            ...
            self.QPushButtonLoadAllTags.clicked.connect(
    ...
    def load_io_tree(self):
        ...
        try:
            ...
            if pidtank_sensors is not None:
                self._load_simulation_signals(
                    pidtank_sensors, "PIDtankValve_Sensors_Actuators")

            pidtank_controls = root.find('PIDtankValve_PLC_Controls')
            if pidtank_controls is not None:
                self._load_simulation_signals(
                    pidtank_controls, "PIDtankValve_PLC_Controls")
        ...
        try:
            ...
            if not all_signals:
                QMessageBox.information(
                    self, "No Tags", "No tags found to load")
                return
            ...
            for current_row, (signal_name, signal_data) in enumerate(all_signals):
                ...
                table.remove_duplicate_signals(
                    signal_name, exclude_row=current_row)

                # Set signal name
                table.setItem(current_row, 0,
                              EditableTableWidgetItem(signal_name))
                ...
                data_type = signal_data.get('type', 'bool')
                table.setItem(current_row, 1,
                              ReadOnlyTableWidgetItem(data_type))

                io_prefix = signal_data.get('io_prefix', 'I')
                byte_num, bit_num, full_address = table.find_free_address(
                    io_prefix, data_type)
                table.setItem(current_row, 2,
                              EditableTableWidgetItem(str(byte_num)))
                if bit_num is not None:
                    table.setItem(current_row, 3,
                                  EditableTableWidgetItem(str(bit_num)))
                else:
                    table.setItem(current_row, 3, EditableTableWidgetItem(""))
                table.setItem(current_row, 4,
                              ReadOnlyTableWidgetItem(full_address))
                ...
                if 'description' in signal_data:
                    table.setItem(current_row, 6, ReadOnlyTableWidgetItem(
                        signal_data['description']))
                if 'range' in signal_data:
                    table.setItem(current_row, 7, ReadOnlyTableWidgetItem(
                        signal_data['range']))
            ...
            QMessageBox.information(
                self, "Success", f"Loaded {len(all_signals)} tags into table")

        except Exception as e:
            QMessageBox.critical(
                self, "Error", f"Failed to load tags: {str(e)}")
        ...
        try:
            ...
            try:
                ...
                if 'offsets' in config_data:
                    ...
                    self.QLineEdit_BoolInput.setText(
                        str(config_data['offsets'].get('BoolInput', 0)))
                    self.QLineEdit_BoolOutput.setText(
                        str(config_data['offsets'].get('BoolOutput', 0)))
                    self.QLineEdit_DWORDInput.setText(
                        str(config_data['offsets'].get('DWORDInput', 2)))
                    self.QLineEdit_DWORDOutput.setText(
                        str(config_data['offsets'].get('DWORDOutput', 2)))
                ...
                for idx, signal in enumerate(config_data['signals']):
                    ...
                    table.setItem(idx, 0, EditableTableWidgetItem(
                        signal.get('name', '')))
                    table.setItem(idx, 1, ReadOnlyTableWidgetItem(
                        signal.get('type', '')))
                    table.setItem(idx, 2, EditableTableWidgetItem(
                        signal.get('byte', '')))
                    table.setItem(idx, 3, EditableTableWidgetItem(
                        signal.get('bit', '')))
                    table.setItem(idx, 4, ReadOnlyTableWidgetItem(
                        signal.get('address', '')))
                    table.setItem(idx, 5, ReadOnlyTableWidgetItem(
                        signal.get('status', '')))
                    table.setItem(idx, 6, ReadOnlyTableWidgetItem(
                        signal.get('description', '')))
                    table.setItem(idx, 7, ReadOnlyTableWidgetItem(
                        signal.get('range', '')))
    ...
    def reload_io_config(self, skip_confirmation=False):
        ...
        try:
            if not hasattr(self, 'tanksim_config') or self.tanksim_config is None:
                QMessageBox.warning(
                    self, "Error", "TankSim config unavailable")
                return
            ...
            try:
                if hasattr(self, 'mainConfig') and hasattr(self.mainConfig, 'ioHandler'):
                    ...
                    logger.info(
                        "Configuration reloaded - starting 500ms IO initialization period")
            except Exception as e:
                logger.warning(f"Could not start force write period: {e}")

            QMessageBox.information(
                self, "Success", "Configuration activated and reloaded successfully")
            ...
            QMessageBox.critical(
                self, "Error", f"Failed to activate/reload: {str(e)}")
            ...
            for row in range(table.rowCount()):
                ...
                if value is None:
                    ...
                    # PID Valve Controls - Specific handling for known PID controls
                        ...
                        value = bool(
                            getattr(status, 'pidPidValveStartCmd', False))
                    elif attr_name == "DIPidValveStop":
                        value = bool(
                            getattr(status, 'pidPidValveStopCmd', False))
                    elif attr_name == "DIPidValveReset":
                        value = bool(
                            getattr(status, 'pidPidValveResetCmd', False))
                    elif attr_name == "DIPidValveAuto":
                        value = bool(
                            getattr(status, 'pidPidValveAutoCmd', True))
                    elif attr_name == "DIPidValveMan":
                        value = bool(
                            getattr(status, 'pidPidValveManCmd', False))
                    elif attr_name == "DIPidTankValveAItemp":
                        value = bool(
                            getattr(status, 'pidPidTankValveAItempCmd', False))
                    elif attr_name == "DIPidTankValveDItemp":
                        value = bool(
                            getattr(status, 'pidPidTankValveDItempCmd', False))
                    elif attr_name == "DIPidTankValveAIlevel":
                        value = bool(
                            getattr(status, 'pidPidTankValveAIlevelCmd', False))
                    elif attr_name == "DIPidTankValveDIlevel":
                        value = bool(
                            getattr(status, 'pidPidTankValveDIlevelCmd', False))
                    elif attr_name == "AIPidTankTempSP":
                        value = int(
                            getattr(status, 'pidPidTankTempSPValue', 0))
                    elif attr_name == "AIPidTankLevelSP":
                        value = int(
                            getattr(status, 'pidPidTankLevelSPValue', 0))
                    ...
                    # Handler creates attributes like pidPidValveStartCmd for digital, pidPidTankTempSPValue for analog
                        ...
                        attr_to_check = f"pid{attr_name[2:]}Cmd" if attr_name.startswith(
                            "DI") else f"pid{attr_name[2:]}Value"
    ...
    def handle_io_signal_rename(self, canonical: str, old_display: str, new_display: str):
        ...
        # Refresh SVG tag labels in the simulation widget
        try:
            if hasattr(self, 'vat_widget') and self.vat_widget:
                if hasattr(self.vat_widget, 'update_tag_labels'):
                    self.vat_widget.update_tag_labels()
        except Exception:
            pass
