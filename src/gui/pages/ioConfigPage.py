...
class DroppableTableWidget(QTableWidget):
    ...
    def show_force_menu(self, row, signal_name, data_type, position):
        ...
        if row in self.forced_rows:
            remove_action = QAction(
                f"🔓 Remove Force from '{signal_name}'", self)
            ...
            if data_type == 'bool':
                ...
                force_value = QAction(
                    f"🔒 Force '{signal_name}' to value...", self)
    ...
    def _save_row_data(self, row):
        ...
        # If the name (column 0) was changed, update VatWidget tags
        if col == 0 and self.io_screen and hasattr(self.io_screen, '_update_vat_widget_tags'):
            try:
                self.io_screen._update_vat_widget_tags()
            except Exception:
                pass
                ...
                try:
                    ...
                    if signal_data:
                        ...
                        if 'status' in signal_data:
                            self.setItem(row, 5, ReadOnlyTableWidgetItem(
                                signal_data['status']))
...
class IOConfigMixin:
    ...
    def save_io_configuration(self):
        ...
        try:
            ...
            # Update VatWidget tags after successful save
            self._update_vat_widget_tags()
    ...
    def load_io_configuration(self):
        ...
        try:
            ...
            for idx, signal in enumerate(config_data['signals']):
                ...
                table.setItem(idx, 0, ReadOnlyTableWidgetItem(
                    signal.get('name', '')))
            ...
            self.io_screen.save_configuration()
            # Update VatWidget tags after loading new configuration
            self._update_vat_widget_tags()
    ...
    def reload_io_config(self):
        ...
        try:
            ...
            # Update VatWidget tags after reloading configuration
            self._update_vat_widget_tags()
    ...
    def _update_vat_widget_tags(self):
        """
        Update VatWidget tag text based on current IO configuration.
        This method syncs the SVG tag names with the configured signal names.
        """
        try:
            # Get the active simulation's VatWidget
            if not hasattr(self, 'mainConfig') or self.mainConfig is None:
                return

            if not hasattr(self.mainConfig, 'simulationManager'):
                return

            active_sim = self.mainConfig.simulationManager.get_active_simulation()
            if active_sim is None:
                return

            # Check if the active simulation has a gui_widget (VatWidget)
            if not hasattr(active_sim, 'gui_widget') or active_sim.gui_widget is None:
                return

            vat_widget = active_sim.gui_widget

            # Check if it's a VatWidget (has update_tag_text_from_config method)
            if not hasattr(vat_widget, 'update_tag_text_from_config'):
                return

            # Read current signal configuration from the table
            table = self.tableWidget_IO
            signal_name_mapping = {}

            # Build mapping of default signal names to configured names
            default_to_configured = {
                'Heater': 'Heater',
                'TemperatureSensor': 'TemperatureSensor',
                'ValveIn': 'ValveIn',
                'ValveOut': 'ValveOut',
                'LevelSensor': 'LevelSensor',
                'LevelSensorHigh': 'LevelSensorHigh',
                'LevelSensorLow': 'LevelSensorLow',
            }

            # Check table for any customized signal names
            for row in range(table.rowCount()):
                name_item = table.item(row, 0)
                if name_item and name_item.text():
                    signal_name = name_item.text()
                    # If this is one of our tracked signals, map it
                    for default_name in default_to_configured.keys():
                        if signal_name == default_name or signal_name.lower() == default_name.lower():
                            default_to_configured[default_name] = signal_name

            # Call the VatWidget's update method with the mapping
            vat_widget.update_tag_text_from_config(default_to_configured)

        except Exception as e:
            # Silently fail if VatWidget update cannot be performed
            pass
