...
class ioHandler:
    ...
    def updateIO(self, plc: logoS7, mainConfig: mainConfigClass, config: configurationClass,
        ...
        if "DQValveIn" in forced_values:
            ...
            if debug_this_cycle:
                print(
                    f"   ValveInFraction FORCED: {status.valveInOpenFraction}")
        ...
        if "DQValveOut" in forced_values:
            ...
            if debug_this_cycle:
                print(
                    f"   ValveOutFraction FORCED: {status.valveOutOpenFraction}")
        elif (mainConfig.plcGuiControl == "plc"):
        ...
        if "DQHeater" in forced_values:
            ...
            if debug_this_cycle:
                print(
                    f"   HeaterFraction FORCED: {status.heaterPowerFraction}")
        elif (mainConfig.plcGuiControl == "plc"):
        ...
        if "DQValveIn" in forced_values:
            plc.SetDO(config.DQValveIn["byte"], config.DQValveIn["bit"], bool(
                forced_values["DQValveIn"]))
        if "AQValveInFraction" in forced_values:
            plc.SetAO(config.AQValveInFraction["byte"], int(
                forced_values["AQValveInFraction"]))

        if "DQValveOut" in forced_values:
            plc.SetDO(config.DQValveOut["byte"], config.DQValveOut["bit"], bool(
                forced_values["DQValveOut"]))
        if "AQValveOutFraction" in forced_values:
            plc.SetAO(config.AQValveOutFraction["byte"], int(
                forced_values["AQValveOutFraction"]))

        if "DQHeater" in forced_values:
            plc.SetDO(config.DQHeater["byte"], config.DQHeater["bit"], bool(
                forced_values["DQHeater"]))
        if "AQHeaterFraction" in forced_values:
            plc.SetAO(config.AQHeaterFraction["byte"], int(
                forced_values["AQHeaterFraction"]))
        ...
        if "DILevelSensorHigh" in forced_values:
            ...
            if debug_this_cycle:
                print(
                    f"   DILevelSensorHigh FORCED: {value} @ {config.DILevelSensorHigh}")
        else:
        ...
        if "DILevelSensorLow" in forced_values:
            ...
            if debug_this_cycle:
                print(
                    f"   DILevelSensorLow FORCED: {value} @ {config.DILevelSensorLow}")
        else:
        ...
        if "AILevelSensor" in forced_values:
            ...
            if debug_this_cycle:
                print(
                    f"   AILevelSensor FORCED: {value} @ byte {config.AILevelSensor['byte']}")
        else:
            value = int(self.mapValue(0, config.tankVolume, 0,
                        plc.analogMax, status.liquidVolume))
            if debug_this_cycle:
                print(
                    f"   AILevelSensor: vol={status.liquidVolume:.1f}mm -> PLC={value} @ byte {config.AILevelSensor['byte']}")
        ...
        if "AITemperatureSensor" in forced_values:
            ...
            if debug_this_cycle:
                print(
                    f"   AITemperatureSensor FORCED: {value} @ byte {config.AITemperatureSensor['byte']}")
        else:
            value = int(self.mapValue(-50, 250, 0,
                        plc.analogMax, status.liquidTemperature))
            if debug_this_cycle:
                print(
                    f"   AITemperatureSensor: temp={status.liquidTemperature:.1f}C -> PLC={value} @ byte {config.AITemperatureSensor['byte']}")
